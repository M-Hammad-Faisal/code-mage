...
## Mapping This to What Jest's API Actually Calls Things

The terminology gets genuinely confusing in practice because the tools themselves don't use Meszaros's five words consistently. Jest's own API is the clearest example: `jest.fn()` — the function most people reach for and call "a mock" — is really closer to a **stub-that-also-spies**, not a Meszaros-precise mock at all.

```typescript
const chargeFn = jest.fn().mockResolvedValue({ success: true, transactionId: 'txn_123' });
// stub behavior: returns a canned answer every time, regardless of arguments

await orderService.checkout(cart, { charge: chargeFn });

expect(chargeFn).toHaveBeenCalledWith(2999); // spy behavior: inspecting recorded calls, after the fact
```

That single `jest.fn()` call does both a stub's job (`mockResolvedValue` sets the canned return) and a spy's job (`toHaveBeenCalledWith` inspects the call log afterward) — which is exactly why "jest mock" gets used as a catch-all term covering what Meszaros's taxonomy treats as two distinct concepts. `jest.spyOn(object, 'methodName')` is the one Jest API that maps cleanly onto Meszaros's actual **Spy** definition — it wraps a *real* existing method, letting it run by default while recording how it was called, and only stubs its behavior if you explicitly chain `.mockImplementation()` onto it:

```typescript
const spy = jest.spyOn(paymentGateway, 'charge'); // real method still runs
await orderService.checkout(cart, paymentGateway);
expect(spy).toHaveBeenCalledWith(2999); // recorded, but nothing was faked
```

Neither Jest nor most JavaScript testing libraries ship a genuine Meszaros-style **Mock** — upfront expectations, automatic failure the moment reality diverges — as a first-class primitive the way, say, a Java mocking framework built around that exact concept does. What you get instead is `jest.fn()`'s stub-plus-spy hybrid, composed with assertions you write yourself afterward, which is functionally closer to state/spy-style verification even when someone calls the whole thing "mocking." This is worth knowing specifically because it means the tool's own vocabulary won't rescue you from the ambiguity covered above — "I mocked it with `jest.fn()`" is still exactly as underspecified a sentence as "I mocked it," because `jest.fn()` itself doesn't commit you to stub-style or spy-style usage; that's a decision you make in how you call it and what you assert on afterward, not something the API name settles for you.
