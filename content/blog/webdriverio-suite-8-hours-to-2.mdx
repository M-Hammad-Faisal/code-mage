...
The "parallelization alone would have gotten us to ~3.5 hours, not 1 hour" number isn't a rough guess — it's roughly what **Amdahl's Law** predicts, the standard formula for exactly this situation: given a fraction of work that's inherently serial and can't be split across workers, speedup from adding parallel workers is bounded by S = 1/(f + (1-f)/N), where f is the serial fraction and N is the number of parallel workers. As N grows large, that formula approaches a hard ceiling of 1/f — meaning even infinite parallel Grid nodes can't beat whatever fraction of the suite's total work genuinely can't be parallelized. In our case, that serial fraction was real and specific: every worker still had to independently pay the cost of its own browser startup, its own login flow, its own test-data seeding — work that doesn't shrink just because you've added more nodes running it *simultaneously*. That's exactly why consolidating the redundant setup (Fix 1) and adding concurrency (Fix 2) compounded instead of just adding — cutting duplicated setup reduced *f itself*, the serial fraction each parallel worker was stuck paying, which is a fundamentally different lever than adding more workers to run the same amount of serial-per-worker cost concurrently.
