...
## What the Abstraction Habit Actually Looked Like in Practice, Three Times Over

It's easy to say "I learned to design good abstractions" as a career-narrative line and leave it vague enough to mean nothing specific. Here's what it actually meant, concretely, the three separate times it showed up in real, shipped work — written up individually elsewhere on this blog, but worth naming together here because seeing the same shape three times, across three genuinely different problem domains, is the actual evidence for the claim, not just an assertion about it.

The WebdriverIO multi-platform framework wasn't "one codebase that happens to also run on mobile" — it was a factory pattern specifically built so one shared test suite could drive web, Android, and iOS, instead of the team maintaining three separate suites with 80% identical logic duplicated across each one. Part of what made that design non-trivial: session setup cost is genuinely different per platform — a headless Chrome session starts in under a second, while an Android emulator cold-booting through Appium can take 30 to 90 seconds before the first command even runs — so the abstraction had to account for that asymmetry rather than pretending all three platforms were equally cheap to spin up.

The payer-integration consolidation was the same underlying move applied to backend integrations instead of test infrastructure: 23 separate bot integrations, each originally built differently, each with its own distinct way of failing, unified into one shared system under real deadline pressure rather than the comfortable, unhurried conditions a "let's refactor this properly" project usually gets.

The anti-detection scraping engine, covered in far more technical depth elsewhere on this blog, needed the same instinct at yet another layer: network-level, browser-level, and behavioral-level detection countermeasures, unified behind one coherent engine rather than built as isolated, ad-hoc patches applied every time a new detection method showed up. Three domains — mobile test infrastructure, backend payment integrations, adversarial browser automation — and the same underlying design question asked of all three: what's the one abstraction that makes the next ten cases easy, instead of each one becoming its own special case that has to be understood and maintained independently.
