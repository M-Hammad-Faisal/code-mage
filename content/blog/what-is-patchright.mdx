...
The biggest change: instead of calling `Runtime.enable` (which is highly detectable), Patchright executes JavaScript through isolated `ExecutionContexts`. You get the exact same result for your code — `page.evaluate()`, locators, everything works identically. But the detection signal is entirely gone.
...
Run both against [Pixelscan](https://pixelscan.net/) — a public page that checks every single automation signal it can find.
...
Patchright isn't the only stealth-patched fork anymore, either. `rebrowser-patches` takes a similar CDP-minimal approach (skipping `Runtime.enable` via `Page.createIsolatedWorld`), and Camoufox does the same job on Firefox instead of Chromium if you need a non-Chromium engine. There's also `invisible_playwright`, a newer entrant with its own published comparison against `rebrowser-patches` for anyone weighing the tradeoffs directly rather than taking either project's word for it. Worth comparing if Patchright alone doesn't get you past a particular target — but Patchright remains the most widely used option and the one with the most active community around it, and remained actively maintained with real releases as recently as August 2026, which matters for a stealth tool specifically since detection techniques evolve and an abandoned fork stops being undetectable the moment sites catch up to whatever it was patching.

One more genuinely useful detail worth knowing if you're going deeper than the basic import swap: Patchright exposes an `isolated_context` option (on by default) that lets you explicitly control whether your `page.evaluate()` calls run in the page's main JavaScript context or a fully isolated one. The isolated default is what makes the stealth work — it's the same mechanism keeping `Runtime.enable` out of the picture — but there are legitimate cases (needing to read or modify a global variable the page itself defined, for instance) where you need main-context access despite the detection tradeoff, and knowing the flag exists means you don't have to choose between "fully stealthy" and "can't do this one specific thing" as an all-or-nothing decision.
...
- **Random delays** — Not `waitForTimeout(1000)` on every action. Actual, organic randomness. I use `Math.random() * 2000 + 500` so each wait is uniquely different.
- **Fresh contexts** — Launch a new browser context per session. Don't carry cookies or state between different scraping accounts.
- **Headed mode** — Always. Even with Patchright, headed vs headless makes a highly detectable difference. In CI, use `xvfb-run` to run headed without needing a physical display.
...
If you're new to Playwright entirely, definitely start with the [Playwright tutorial](/tutorial/playwright) on this site — learn the API extremely well first, then come back here when you actually need stealth.