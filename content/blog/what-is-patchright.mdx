...
Patchright isn't the only stealth-patched fork anymore, either. `rebrowser-patches` takes a similar CDP-minimal approach (skipping `Runtime.enable` via `Page.createIsolatedWorld`), and Camoufox does the same job on Firefox instead of Chromium if you need a non-Chromium engine. Worth comparing if Patchright alone doesn't get you past a particular target — but Patchright remains the most widely used option and the one with the most active community around it.
