...
## The Principle Has a Name, and It Comes From Testing Library

Everything above is a restatement, in first-principles form, of something Testing Library states directly as its guiding principle: ["the more your tests resemble the way your software is used, the more confidence they can give you."](https://testing-library.com/docs/queries/about/#priority) Their own priority list runs `getByRole` first ("your top preference for just about everything"), then `getByLabelText` and `getByPlaceholderText` for form fields specifically, then `getByText` for non-interactive content, down through semantic queries like `getByAltText`, with `getByTestId` explicitly reserved for "when you can't match by role, text, or other semantic queries." That's the same hierarchy argued for above, arrived at independently by a different testing ecosystem — which is itself evidence the reasoning is sound rather than idiosyncratic to Playwright's own docs.

The framing worth borrowing directly from them: a role-based query isn't ranked first because it's *convenient*, it's ranked first because a real user — including one using a screen reader, a switch device, or voice control — finds and operates elements by role and accessible name, not by CSS class or DOM position. A test that queries the same way is testing something closer to the actual user experience, not just an implementation detail that happens to currently be true. Every tier down the hierarchy is a step further from "how a real user actually finds this thing" — which is the same "coupled to purpose vs. coupled to implementation" axis from above, just stated from the user's side of the interaction instead of the DOM's.
