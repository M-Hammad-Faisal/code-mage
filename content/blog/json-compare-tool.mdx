...
## The Better Array Algorithm I Didn't Build (Yet), and Why It's Genuinely Harder

Positional (index-for-index) array diffing has a specific, nameable failure beyond the reorder case already covered: a single insertion near the start of an array cascades into every element after it looking "changed," even though nothing about those elements actually changed — they just moved one position over. Insert one item at index 0 of a 10-element array, and a naive positional diff reports 10 changes instead of the one that actually happened.

The real fix for this is exactly the algorithm behind `git diff` and most mature line-based diff tools: Eugene Myers's 1986 diff algorithm, built on computing the **longest common subsequence** (LCS) between two sequences — the longest set of elements that appear in both, in the same relative order, without needing to be contiguous. Once you have the LCS, everything in it is "unchanged, possibly at a different position," and everything *not* in it is a genuine insertion or deletion — which correctly isolates one real insertion as one change, instead of shifting every subsequent element into "changed" territory.

I didn't build this for the JSON tool, and it's worth being honest about why, rather than pretending it's an obvious next step: LCS-based diffing is a meaningfully harder algorithm than positional comparison — Myers's own algorithm runs in O(ND) time, where N is the combined sequence length and D is the size of the actual edit — and applying it correctly to arrays of *objects*, not just arrays of primitive lines the way `git diff` operates, means first defining what "the same element, just moved" even means for two JSON objects that aren't identical (do they need to be exactly equal to count as "the same element," or just similar enough by some key?). That's a real design decision with no single right answer, which is a large part of why the pragmatic, honest choice was positional diffing with clear reorder detection layered on top, rather than a half-implemented LCS approach that silently got the "are these the same object" question wrong in some cases and right in others.
