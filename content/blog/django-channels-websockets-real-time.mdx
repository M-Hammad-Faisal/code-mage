...
## The Part I Glossed Over: What Actually Runs This in Production

Writing the consumer and running `manage.py runserver` locally is deceptively easy — Django's dev server happily serves ASGI too. Production is where the "just add Channels" pitch gets more honest about its cost, because `gunicorn`, the WSGI server most Django deployments already run, cannot serve an ASGI application at all. You need an actual ASGI server in front of it.

The two real options are Daphne — the reference server maintained by the Channels project itself, built to speak HTTP, HTTP/2, and WebSocket — and Uvicorn, the faster, more general-purpose ASGI server with a broader ecosystem outside the Django world. If you're using Channels specifically, Daphne is the safer default, since it's the one the project tests against; Uvicorn works too and is commonly run as a Gunicorn worker class (`gunicorn tasknest.asgi:application -k uvicorn.workers.UvicornWorker`) to get Gunicorn's process management with Uvicorn's speed. Either way, this is a real infrastructure decision, not a config flag — you're choosing and operating a second kind of server process that didn't exist in the plain-WSGI version of the app, on top of adding Redis. Nginx (or whatever reverse proxy you're already running) sits in front of it as usual, terminating TLS and proxying the `/ws/` path to the ASGI server instead of the WSGI one.

I'm flagging this explicitly because it's the part of the "just switch to ASGI" pitch that tutorials tend to skate past. Adding Channels to Task Nest wasn't one dependency (Redis) — it was a second server process to deploy, monitor, and keep alive, on top of Redis being a hard requirement instead of an optional cache. That's the real complexity tax, and it's worth pricing in before committing to WebSockets for a feature that might not need them.
