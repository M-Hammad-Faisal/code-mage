...
test.describe('Inventory page', () => {
  test.beforeEach(async ({ page }) => {
    // This runs before EACH test below. /inventory.html has no real static
    // route on this site, so it's reached by logging in, not by navigating
    // to it directly.
    await page.goto('/');
...
Now you understand what's happening under the hood. Next chapter: what to do when tests are flaky.