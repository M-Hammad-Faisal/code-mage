...
**Playwright MCP** — Playwright also ships as a Model Context Protocol server, letting an AI coding agent drive a real browser directly through structured accessibility data instead of screenshots. You won't need it to follow this tutorial, but it's worth knowing about if you're already working with an AI agent day to day — `npx @playwright/mcp@latest` is the whole install.
