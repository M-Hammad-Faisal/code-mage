...
// Nth match (0-indexed) — SauceDemo's product cards are plain <div>s with
// no ARIA role, so getByRole('listitem') won't match anything on this
// site. Use the data-test attribute the cards actually carry instead.
await page.locator('[data-test="inventory-item"]').nth(2).click();
...
Here's how you'd add the Backpack to the cart using a stable, chained selector. Note the login first — `/inventory.html` isn't a real page you can navigate to directly (this site is a client-routed SPA with no matching static route), so every test needs to log in through `/` first:
...
test('add specific product to cart', async ({ page }) => {
  ...
  // Find the product card containing the Backpack, then click its button
  await page
    .locator('[data-test="inventory-item"]')
    .filter({ hasText: 'Sauce Labs Backpack' })
    .getByRole('button', { name: 'Add to cart' })
...
// Chaining
page.locator('[data-test="inventory-item"]').filter({ hasText: 'Backpack' }).getByRole('button')
page.locator('[data-test="inventory-item"]').nth(0)
```
