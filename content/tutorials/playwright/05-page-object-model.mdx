...
export class InventoryPage {
  ...
  // No goto() here on purpose — /inventory.html has no real static route
  // on this site, so navigating to it directly 404s. You only ever reach
  // this page by logging in through LoginPage first, which is why every
  // test below uses loginPage.login() instead of inventoryPage.goto().

  async addToCart(productName: string) {
    // SauceDemo's product cards are plain <div>s with no ARIA role, so
    // use the data-test attribute the cards actually carry instead of
    // getByRole('listitem').
    await this.page
...
Next chapter: what's actually happening under the hood when you run `npx playwright test` — the test runner, workers, hooks, and the browser lifecycle.