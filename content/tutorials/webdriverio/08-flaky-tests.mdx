...
await $('[data-test="shopping-cart-badge"]').waitForExist({ timeout: 8000 })
...
```typescript
// Both snippets assume you're already logged in and on /inventory.html
// (reached via login — this site has no real static route for that page,
// only a client-side one, so there's nothing to browser.url() back to).

// FLAKY — click happens before JS is ready right after a page transition
await $('[data-test="shopping-cart-link"]').click()
await $('[data-test="checkout"]').click()  // might not respond yet

// RELIABLE — assert a stable landmark before acting
await $('[data-test="shopping-cart-link"]').click()
await expect($('[data-test="checkout"]')).toBeDisplayed()  // waits until the cart page loads
...
To retry only specific tests rather than the whole spec file, add `this.retries(2)` inline:
...
it('should debug flaky interaction', async () => {
  // /inventory.html has no real static route on this site — reach it by
  // logging in through '/', not by navigating to it directly.
  await browser.url('/')
