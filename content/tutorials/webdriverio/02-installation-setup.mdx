...
No separate driver install needed — WDIO auto-manages browser drivers (Chrome, Firefox, Edge, Safari) since v8.14, downloading and starting the matching driver binary for whatever's in `capabilities`.
...
And in `wdio.conf.ts`, register `tsconfig-paths` so the aliases resolve at runtime:
```typescript
// Add to config:
mochaOpts: {
