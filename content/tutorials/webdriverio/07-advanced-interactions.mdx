...
Then, after clicking whatever triggers the download, poll for the file's existence instead of assuming it landed instantly — a download isn't synchronous with the click that started it:

```typescript
import fs from 'fs'
import path from 'path'

await $('[data-test="download-invoice"]').click()

const filePath = path.join(process.cwd(), 'test-downloads', 'invoice.pdf')
await browser.waitUntil(() => fs.existsSync(filePath), {
  timeout: 10000,
  timeoutMsg: 'expected invoice.pdf to be downloaded within 10s',
})
```

For a text-based format, read the file directly afterward to assert on its actual contents, the same as you would for any other file your test needs to inspect.
