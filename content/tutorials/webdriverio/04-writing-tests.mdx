...
await $('[data-test="shopping-cart-badge"]').waitForExist({ timeout: 5000 })
...
describe('Checkout Flow', () => {
  ...
  // No beforeEach navigation here on purpose — before() already logged in
  // and landed on /inventory.html via the app's own redirect, and every
  // test below continues in that same session. Re-visiting /inventory.html
  // directly would 404 (it has no real static route on this site, only a
  // client-side one reached by logging in).
...
*SauceDemo never triggers a native alert/confirm/prompt, so there's nothing on this tutorial's site to run this against — the commands below are correct WebdriverIO API syntax for when your own app does use one.*
