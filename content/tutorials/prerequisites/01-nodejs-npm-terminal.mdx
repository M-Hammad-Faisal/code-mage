...
# Restart your terminal, then install Node 22 (current LTS)
nvm install 22
nvm use 22
...
Install Node 22, not just "18 or higher" — this tutorial covers all three frameworks, and Cypress 16 dropped support for Node 20 entirely. Node 22 is the one version that works across every track ahead, so it's worth installing now rather than discovering the mismatch later when you get to the Cypress chapter.
...
Node isn't installed or your shell can't find it. Try reopening the terminal. If using nvm, run `nvm use 22`.
...
Your Node installation is old (pre-5.2). Update Node: `nvm install 22 && nvm use 22`.
