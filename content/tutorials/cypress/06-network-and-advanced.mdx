...
Access them in tests with `cy.env()` — this keeps secrets in the Node process instead of exposing them to the browser (`Cypress.expose()` is the newer alternative, but only for values that are safe to leak into browser-side code). Like any `cy.*` command, it's chainable rather than awaitable — resolve the value with `.then()`:
