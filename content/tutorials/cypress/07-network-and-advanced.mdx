...
*SauceDemo never triggers a native alert/confirm/prompt, so there's nothing on this tutorial's site to run this against — the commands below are correct Cypress syntax for when your own app does use one.*
...
*SauceDemo has no file upload input anywhere in the app — this section is reference syntax, verified against Cypress's own docs, not something you can run against this tutorial's site.*

Cypress has built-in file upload support via `.selectFile()` — no plugin needed:

```typescript
// In your test
cy.get('[data-test="file-input"]').selectFile('cypress/fixtures/test-document.pdf')
...
Access them in tests with `cy.env()` — this keeps secrets in the Node process instead of exposing them to the browser (`Cypress.expose()` is the newer alternative, but only for values that are safe to leak into browser-side code). As of Cypress 16, `cy.env()` only accepts an array of keys and yields an object of `{ key: value }` pairs — there's no single-key `cy.env('username')` getter form anymore. Like any `cy.*` command, it's chainable rather than awaitable — resolve the values with `.then()`:
